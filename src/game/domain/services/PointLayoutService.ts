...
function hasNavigableExit(collisionGrid: CollisionGrid, tile: TilePosition, tileSize: number): boolean {
  const collisionTiles = collisionGrid.getTilesAt(tile);
  return DIRECTIONS.some((direction) => canMove(direction, 0, 0, collisionTiles, tileSize, 'pacman'));
}

function isPlayableNavigationTile(map: WorldMapData, collisionGrid: CollisionGrid, tile: TilePosition, tileSize: number): boolean {
  if (!isWithinBounds(map, tile)) {
    return false;
  }

  const mapTile = map.tiles[tile.y]?.[tile.x];
  if (!mapTile || mapTile.gid === null || mapTile.collision.penGate) {
    return false;
  }

  return hasNavigableExit(collisionGrid, tile, tileSize);
}
...
function resolveTraversalStartTile(
  ...
  if (isPlayableNavigationTile(map, collisionGrid, preferredStart, tileSize)) {
  ...
  for (let y = 0; y < map.height; y += 1) {
    for (let x = 0; x < map.width; x += 1) {
      const candidate = { x, y };
      if (isPlayableNavigationTile(map, collisionGrid, candidate, tileSize)) {
        return candidate;
      }
    }
  }
...
function collectReachableTiles(
  ...
  for (let index = 0; index < queue.length; index += 1) {
    ...
    if (!isPlayableNavigationTile(map, collisionGrid, current, tileSize)) {
      continue;
    }

    reachable.push(current);

    const neighbors = collisionGrid.getTilesAt(current);
    DIRECTIONS.forEach((direction) => {
      if (!canMove(direction, 0, 0, neighbors, tileSize, 'pacman')) {
        return;
      }

      const delta = DIRECTION_VECTORS[direction];
      const nextTile = { x: current.x + delta.dx, y: current.y + delta.dy };
      if (!isWithinBounds(map, nextTile)) {
        return;
      }
...
export function buildPointLayout(params: BuildPointLayoutParams): PointLayout {
  ...
  const basePoints = collectReachableTiles(map, collisionGrid, traversalStart, tileSize);
